...
public enum Events {
    ...
    E_UPDATE_STEP_DEFINITION("Can't found definition `{}` for update"),
    ...
    I_SEND_COMPENSATION_MESSAGE("COMPENSATION: Send compensation on topic `{}`, message `{}`, headers `{}`"),
    I_REQUEST_COMPENSATION("Request for compensation. Saga `{}`"),
    I_START_COMPENSATION("Start compensation. Saga `{}`"),
    I_SAGA_COMPENSATED("Saga already compensated. Saga `{}`"),
    E_SEND_COMPENSATION_MESSAGE("COMPENSATION: Error while sending compensation message"),
